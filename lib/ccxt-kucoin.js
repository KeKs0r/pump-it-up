...
if (process.env.NODE_ENV === 'production' && !process.env.KUCOIN_API_KEY) {
  console.warn('KUCOIN_API_KEY not set') // eslint-disable-line no-console
}
...
async function fetchOrder(orderId, pair, type) {
  if (!this.orders[orderId] && !pair) {
    console.warn('Dont have this order in memory') // eslint-disable-line no-console
    return
  }
