...
function isRelevant(text) {
  ...
  return t.indexOf('The coin is') > -1
}

function pumpNpump(state, em) {
  em.once(state.__events.INIT, () => {
    client.connect(connection => {
      connection.on('message', msg => {
        const { to } = msg
        if (to.peer_id === 1399647465 && isRelevant(msg.text)) {
  ...
  async function checkForImage(text, em) {
    const linkResult = parseLink(text)
    if (linkResult) {
      const options = parseText(linkResult)
      const coin = _.head(_.filter(options, o => o !== 'HOLD'))
      if (coin) {
        em.emit('PARSE:PROPOSE_COIN', coin)
      }
